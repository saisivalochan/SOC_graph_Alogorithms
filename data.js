...
vector<int> mstClustering(int n, vector<tuple<int,int,int>>& edges, int K) {
    ...
    // ============================================================
    {
      ...
      topics: ["Project", "BFS", "DFS", "Dijkstra", "A*", "Backtracking"],
      goal: "Build one C++ program that reads a maze with coins, plans the shortest path that starts at S, collects every coin, and ends at G — using all four graph-search algorithms.",

      concepts: [
        {
          name: "Capstone Project",
          explanation: `A single C++ project that pulls together everything from Weeks 3-6. You will build one program that reads a maze containing coins, plans the shortest total path that starts at S, collects every coin, and ends at G — using four different graph-search algorithms.

The trick: shortest-path-between-two-cells is a subproblem you will solve many times (once per pair of interesting points). The order in which to visit the coins is a separate decision. That split is what forces you to use different algorithms for different parts of the same project.

## Input format (fixed)

Each test case is a single file input.txt with this shape:

Line 1: two integers R C (rows, columns).
Line 2: two integers start_row start_col.
Line 3: two integers goal_row goal_col.
Lines 4 through R+3: R rows of C characters describing the maze.

Cell meanings:

S = start (only one).
G = goal (only one).
c = coin (can be many; up to 8 per maze).
. = open cell (cost 1).
# = wall (impassable).
0 through 9 = terrain cost (only used by Dijkstra and A*; BFS and DFS treat them as cost 1).

Movement is 4-directional (up, down, left, right). You are guaranteed every coin is reachable from S, and G is reachable from every coin.

## What each algorithm must do

Let K = number of coins in the maze. Define the "key points" as the set S plus every coin plus G, so there are K+2 key points total.

BFS: for the uniform-cost run, compute the shortest step-distance between every pair of key points. Result is a (K+2) by (K+2) matrix. This means running BFS K+2 times, once per source.

DFS backtracking: enumerate all K! permutations of coin-visit order. For each permutation, sum dist[S][first] + dist[first][second] + ... + dist[last][G] using the matrix from BFS. Track the minimum and prune any partial sum already greater than or equal to the current best. Run DFS twice — once on the BFS matrix (uniform result) and once on the Dijkstra matrix (weighted result).

Dijkstra: for the weighted run, same idea as BFS but with terrain costs, using a priority queue. K+2 Dijkstra calls produce a weighted (K+2) by (K+2) matrix.

A star: same shape as Dijkstra — K+2 A* calls to build the weighted matrix — but using the Manhattan-distance heuristic. Must produce the SAME dist matrix as Dijkstra (and therefore the same best coin order), while expanding FEWER cells across all K+2 queries.

## Output format (fixed)

For each test case, print two blocks — one for the uniform-cost run and one for the weighted run — followed by a single-line summary. See the "Expected output format" code block below for the exact text.

Path visualisation: mark path cells with * but keep S, G, and c characters visible. If any coin is unreachable, print "Path found: no" and skip the rest.

## Code structure (enforced)

Your program must expose exactly the functions shown in the "Enforced C++ function signatures" code block. The neighbours helper must be shared — no copy-pasted grid-neighbour logic across the four algorithms.

The grader checks (by grep and by runtime call-counters) that all four algorithms are actually invoked. If DFS or A* is never called, that section scores zero.

## Test cases

Four test mazes are provided as separate code blocks below. Copy them to test_mazes/ folder. Universal sanity checks that must pass on every test:

dist[i][j] must equal dist[j][i] for all pairs — grids are undirected so the matrix must be symmetric.
dist[i][i] must be zero for all i.
Dijkstra's matrix must be greater than or equal to BFS's matrix cell-by-cell on unweighted inputs.
A*'s matrix must be IDENTICAL to Dijkstra's — same numbers, cell-by-cell.
DFS backtracking on the 8-coin stress test must complete in under 2 seconds.

## Deliverables

1. maze_solver.cpp — one file, compiles with g++ -O2 maze_solver.cpp -o maze_solver.
2. test_mazes folder — the four test files listed below.
3. README.md — ten lines: compile and run commands plus expected output on the tiny test.
4. report.md — half a page: which algorithm did what, A* versus Dijkstra expansion counts, and best coin order for the medium maze.

## Rubric (100 points)

BFS builds correct dist matrix on uniform maze: 15 points.
DFS backtracking finds provably optimal coin order (verify against the 8-coin stress test): 20 points.
Dijkstra builds correct weighted dist matrix: 20 points.
A* matches Dijkstra's matrix exactly AND expands fewer cells total: 20 points.
Enforced structure — all four functions present, all four invoked, single shared neighbours helper: 10 points.
Output format matches spec character-for-character on the tiny maze: 5 points.
report.md explains observed differences with numbers from your runs: 10 points.

## Explicit non-goals

No GUI, no animation. No diagonal movement. No maze generation. K is at most 8 — plain permutation DFS is fine, do not use bitmask DP. No partial coin collection — all coins must be collected.

## Concepts this project touches

Week 3: Queue (BFS), Stack (iterative DFS), Hashing (visited set).
Week 4: Priority Queue (Dijkstra, A*).
Week 5: Graph representations (implicit grid), BFS, DFS.
Week 6: Dijkstra, path reconstruction with parent map.`,
          codeBlocks: [
            {
              title: "Enforced C++ function signatures",
              code: `struct Grid { int R, C; vector<string> cells; };
...
vector<pair<int,int>> neighbours(const Grid& g, int r, int c);`
            ...
            {
              title: "Expected output format (per test case, two blocks + summary)",
              code: `=== UNIFORM COST (BFS + DFS) ===
Coins: 4
Best order: S -> c(2,2) -> c(0,5) -> c(2,6) -> c(4,6) -> G
Total steps: 22
Path:
S**#.c..
.#*#.#..
...
=== WEIGHTED (Dijkstra + A*) ===
Coins: 4
Best order: S -> c(0,5) -> c(2,2) -> c(2,6) -> c(4,6) -> G
Total cost: 31
A* cells expanded across all pair queries: 87
Dijkstra cells expanded across all pair queries: 142

SUMMARY | BFS pairs computed: 20 | DFS orderings tried: 24 | Dijkstra:142 | A*:87`
            },
            {
              title: "Test 1 — tiny_2coins.txt (hand-verifiable)",
              code: `5 8
0 0
4 7
S......c
...
.......G`
            ...
            {
              title: "Test 2 — medium_5coins.txt",
              code: `10 10
0 0
9 9
S......c..
...
....##...G`
            ...
            {
              title: "Test 3 — weighted_4coins.txt (Dijkstra vs A* must beat BFS on cost)",
              code: `6 8
0 0
5 7
S1.....c
...
c111111G`
            ...
            {
              title: "Test 4 — stress_8coins.txt (must complete in under 2s with pruning)",
              code: `15 15
0 0
14 14
S........#....c
...
c............cG`
          ...
          keyPoints: [
            "You must implement all four algorithms; each has a specific role in the pipeline.",
            "BFS builds the unweighted (K+2)x(K+2) distance matrix between S, all coins, and G.",
            "Dijkstra + A* build the weighted distance matrix; both must produce the same numbers.",
            "DFS enumerates all K! coin orderings with best-so-far pruning; feed it the matrix from BFS or Dijkstra.",
            "K is at most 8, so plain permutation DFS suffices — no bitmask DP required.",
            "Deliver: single .cpp file + four test mazes + README + half-page report.",
            "Grader checks all four algorithms are actually invoked (not just present in code).",
            "A* expanded-count MUST be strictly less than Dijkstra's on at least one non-trivial test."
          ],
          pitfalls: [
            "Running DFS on the coin order without best-so-far pruning — 8! orderings times out.",
            "A* returning a different matrix than Dijkstra — heuristic is inadmissible or wrong.",
            "Reading digit costs with (int)ch instead of ch - '0' — '0' is ASCII 48, not 0.",
            "Path length (in cells) is different from total cost (sum of cell weights) — the output block asks for both.",
